...
@section('konten')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('berkas.edit', $data['id']) }}" method="POST" autocomplete="off">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="namaBerkas" placeholder="Masukkan Nama Berkas"
                        name="namaBerkas" value="{{ old('namaBerkas') ?? $data['nama_berkas'] }}" REQUIRED>
                    @error('namaBerkas')
                        <div class="text-danger mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="retensiAktif" placeholder="Masukkan Retensi Aktif"
                        name="retensiAktif" value="{{ old('retensiAktif') ?? $data['retensi_aktif'] }}" REQUIRED>
                    @error('retensiAktif')
                        <div class="text-danger mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="retensiInaktif" placeholder="Masukkan Retensi Aktif"
                        name="retensiInaktif" value="{{ old('retensiInaktif') ?? $data['retensi_inaktif'] }}" REQUIRED>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="keteranganAkhir" name="keteranganAkhir" required>
                        <option value="Permanen" {{ old('keteranganAkhir', $data['keterangan_akhir'] ?? 'Permanen') == 'Permanen' ? 'selected' : '' }}>Permanen</option>
                        <option value="Musnah" {{ old('keteranganAkhir', $data['keterangan_akhir'] ?? 'Permanen') == 'Musnah' ? 'selected' : '' }}>Musnah</option>
                    </select>
