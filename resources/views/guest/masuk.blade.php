...
@push('js')
    <script>
        $(function () {
            ...
            const $results = $('#listSuratMasuk');
            const $pagination = $('#paginationListSuratMasuk');
            ...
            function renderCard(data) {
                return '<article class="guest-document-card">' +
                    '<div class="guest-document-top">' +
                        '<span class="guest-document-type"><i class="fas fa-inbox"></i> Surat Masuk</span>' +
                        accessBadge(data) +
                    '</div>' +
                    '<div class="guest-document-body">' +
                        ...
                        '<div class="guest-document-meta">' +
                            ...
                            '<div class="guest-meta-item"><span>Agenda</span><strong>' + (data.is_srikandi ? 'SRIKANDI' : escapeHtml(data.nomor_agenda || '-')) + '</strong></div>' +
                            '<div class="guest-meta-item"><span>Tanggal surat</span><strong>' + escapeHtml(data.tanggal_surat) + '</strong></div>' +
                            '<div class="guest-meta-item"><span>Diterima</span><strong>' + escapeHtml(data.tanggal_diterima) + '</strong></div>' +
                        '</div>' +
                    '</div>' +
                    '<div class="guest-document-footer">' +
                        '<span class="guest-document-owner" title="' + escapeHtml(data.pengirim) + '">' +
                            '<i class="fas fa-building"></i>' + escapeHtml(data.pengirim) +
                        '</span>' +
                        documentAction(data) +
                    '</div>' +
                '</article>';
            ...
            function renderEmpty() {
                $results.html(
                    '<div class="guest-empty-state">' +
                        ...
                        '<p>Coba gunakan nomor surat, nama pengirim, atau kata kunci perihal yang berbeda.</p>' +
                    '</div>'
                );
            }
...
@section('konten')
    <section class="guest-page-hero">
        <div class="container">
            ...
            <h1>Surat Masuk</h1>
            <p>Telusuri naskah yang diterima berdasarkan nomor surat, pengirim, atau perihal.</p>
        </div>
    </section>
    ...
    <section class="guest-page-content">
        <div class="container">
            <div class="guest-search-panel">
                <label for="pencarian" class="sr-only">Cari surat masuk</label>
                <div class="guest-search-field">
                    <i class="fas fa-search" aria-hidden="true"></i>
                    <input type="search" class="form-control guest-search-input" id="pencarian"
                        placeholder="Cari nomor surat, pengirim, atau perihal..." autocomplete="off">
            ...
            <div class="guest-document-grid" id="listSuratMasuk">
                @forelse ($suratMasuk as $data)
                    <article class="guest-document-card">
                        ...
                        <div class="guest-document-body">
                            ...
                            <div class="guest-document-meta">
                                ...
                                <div class="guest-meta-item"><span>Agenda</span><strong>{{ $data->is_srikandi ? 'SRIKANDI' : ($data->nomor_agenda ?: '-') }}</strong></div>
                                <div class="guest-meta-item"><span>Tanggal surat</span><strong>{{ $data->tanggal_surat ?: '-' }}</strong></div>
                                <div class="guest-meta-item"><span>Diterima</span><strong>{{ $data->tanggal_diterima ?: '-' }}</strong></div>
                            </div>
                        </div>
                        <div class="guest-document-footer">
                            <span class="guest-document-owner" title="{{ $data->pengirim }}">
                                <i class="fas fa-building"></i>{{ $data->pengirim ?: '-' }}
                            </span>
                ...
                @empty
                    <div class="guest-empty-state">
                        <span class="guest-empty-icon"><i class="fas fa-inbox"></i></span>
                        <h3>Belum ada surat masuk</h3>
                        <p>Koleksi surat masuk belum tersedia.</p>
                    </div>
                @endforelse
            </div>

            <div class="guest-pagination" id="paginationListSuratMasuk">
                {{ $suratMasuk->onEachSide(1)->links() }}
            </div>
        </div>
    </section>
@endsection
